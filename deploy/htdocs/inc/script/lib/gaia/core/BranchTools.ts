...
class BranchTools
{
	 ...
	 * @returns {string}
	 */
	public static getValidRoute(route:string):string
	{
		// strip the querystring from the branch
		route = route.split('?').shift();

		// get route segments
		var routeArray:string[] = route.split("/");

		// lookup shortcut
		var routes:any = SiteModel.getRoutes();

		// walks trough the route segments
		while (routeArray.length > 0)
		{
			route = routeArray.join('/');

			// walk trough the valid routs
	 ...
	 * @returns {string}
	 */
	public static getRouteDeeplink(route:string):string
	{
		var validRoute:string = BranchTools.getValidRoute(route);
