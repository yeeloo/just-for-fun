...
define(["require", "exports", 'lib/gaia/events/GaiaEvents', 'lib/temple/events/EventDispatcher'], function (require, exports, gEvents, EventDispatcher) {
    /**
     * @namespace gaia.core
    ...
    var TransitionController = (function (_super) {
        ...
        TransitionController.prototype.transitionOut = function (pages) {
            if (this._debug) {
                console.log('TransitionController::transitionOut()', pages);
            }
        ...
        TransitionController.prototype.transition = function (oldPages, newPages) {
            if (this._debug) {
                console.log('TransitionController::transition', oldPages, newPages);
            }
            this.isInterrupted = false;
        ...
        TransitionController.prototype.transitionIn = function (pages) {
            if (this._debug) {
                console.log('TransitionController::transitionIn(' + pages + ')');
            }
        ...
        TransitionController.prototype.onTransitionOutComplete = function (event) {
            event.target.removeEventListener(gEvents.PageEvent.TRANSITION_OUT_COMPLETE, this._onTransitionOutCompleteDelegate);
            if (this._debug) {
                console.log("TransitionController::transitionOutComplete: \"" + event.target.getBranch() + "\"");
            }
        ...
        TransitionController.prototype.onTransitionInComplete = function (event) {
            if (this._debug) {
                console.log("TransitionController::transitionInComplete: \"" + event.target.getBranch() + "\"");
            }
            event.target.removeEventListener(gEvents.PageEvent.TRANSITION_IN_COMPLETE, this._onTransitionInCompleteDelegate);
        ...
        TransitionController.prototype.onTransitionComplete = function (event) {
            if (this._debug) {
                console.log("TransitionController::transitionComplete: \"" + event.target.getBranch() + "\"");
            }
            event.target.removeEventListener(gEvents.PageEvent.TRANSITION_COMPLETE, this._onTransitionCompleteDelegate);
        ...
        TransitionController.prototype.pageOut = function (page) {
            if (this._debug) {
                console.log("TransitionController::transitionOut: \"" + this.outPages[this.outIndex].getBranch() + "\"");
            }
            if (typeof (page) == 'undefined') {
                page = this.outPages[this.outIndex];
            }
            page.addEventListener(gEvents.PageEvent.TRANSITION_OUT_COMPLETE, this._onTransitionOutCompleteDelegate);
            page.transitionOut();
        };
        TransitionController.prototype.pageIn = function (page) {
            if (this._debug) {
                console.log("TransitionController::transitionIn: \"" + this.inPages[this.inIndex].getBranch() + "\"");
            }
            if (typeof (page) == 'undefined') {
                page = this.inPages[this.inIndex];
            }
            page.addEventListener(gEvents.PageEvent.TRANSITION_IN_COMPLETE, this._onTransitionInCompleteDelegate);
            page.transitionIn();
        };
        TransitionController.prototype.pageTransition = function (page) {
            if (this._debug) {
                console.log("TransitionController::transition: \"" + this.tPages[this.tIndex].getBranch() + "\"");
            }
            if (typeof (page) == 'undefined') {
                page = this.tPages[this.tIndex];
            }
            page.addEventListener(gEvents.PageEvent.TRANSITION_COMPLETE, this._onTransitionCompleteDelegate);
            page.initPage();
            page.transition();
        };
        TransitionController.prototype.interrupt = function () {
            if (!this.isInterrupted && this.transitionState > 0) {
                ...
                console.log(">>> INTERRUPT " + transitionDirection + " <<<");
            }
        };
