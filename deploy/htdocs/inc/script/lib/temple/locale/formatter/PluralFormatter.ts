...
import pl = require('./util/Pluralize');
...
class PluralFormatter implements ITextFormatter
{
	...
	public format(text:string):string
	{
		for (var name in this.replacements)
		{
			if (this.replacements.hasOwnProperty(name))
			{
				...
				text = text.replace(new RegExp('{([^|{}]+)\\|' + name + '(?:\\|([^}]+))?}', 'gmi'), () =>
				{
					return this._pluralize(arguments[1], this.replacements[name], arguments[2]);
				})
			}
		}
	...
	private _pluralize(noun:string, count:any, plural:string = null):string
	{
		...
		return pl.Pluralize.pluralize(noun, count, plural);
	}
}

export = PluralFormatter;