 ...
 */
export function enableGaiaTracking( gaia:Gaia.IGaia ){
	gaia.afterGoto( ( event:gEvents.GaiaEvent ) =>{
		try {
			//		trackPage(gaia.getPage(gaia.getCurrentBranch()).route.base + (event.fullBranch.substr(event.validBranch.length)));
			trackPage( gaia.getPage( gaia.getCurrentBranch() ).route );
		}
		catch( error ) {
